...
const CIRCLE_SELECT = `
  id, title, category, area, description, location, recurring, created_by,
  host:profiles!circles_created_by_fkey ( id, full_name ),
  events (
    id, starts_at, ends_at, cancelled,
    attendees ( profile_id, profiles ( full_name ) )
  )
`;

// Flattens a Supabase circles row (with nested events/attendees) into the
// board's card shape, picking the soonest non-cancelled upcoming event.
function toCard(row) {
  const now = Date.now();
  const upcoming = (row.events || [])
    .filter((e) => !e.cancelled && new Date(e.starts_at).getTime() >= now)
    .sort((a, b) => new Date(a.starts_at) - new Date(b.starts_at));
  const nextEvent = upcoming[0] || null;
  const attendees = nextEvent?.attendees || [];
  return {
    id: row.id,
    title: row.title,
    ...
    recurring: row.recurring || "One-off event",
    host: row.host?.full_name || "A neighbour",
    when: nextEvent?.starts_at || null,
    eventId: nextEvent?.id || null,
    joinedIds: attendees.map((a) => a.profile_id),
    joinedNames: attendees.map((a) => a.profiles?.full_name).filter(Boolean),
  };
}
...
export default function CommunityBoard() {
  ...
  const loadProfile = useCallback(async (userId) => {
    ...
    if (data && !data.area) setShowSettings(true);
  }, []);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("circles-board")
      .on("postgres_changes", { event: "*", schema: "public", table: "circles" }, loadCircles)
      .on("postgres_changes", { event: "*", schema: "public", table: "events" }, loadCircles)
      .on("postgres_changes", { event: "*", schema: "public", table: "attendees" }, loadCircles)
      .subscribe();
  ...
  const saveArea = async (area) => {
    ...
    const { error } = await supabase.from("profiles").update({ area }).eq("id", session.user.id);
    if (!error) {
      setProfile((p) => (p ? { ...p, area } : p));
  ...
  const toggleJoin = async (circle) => {
    ...
    if (!circle.eventId) return;
    const already = circle.joinedIds.includes(session.user.id);
    if (already) {
      await supabase.from("attendees").delete().eq("event_id", circle.eventId).eq("profile_id", session.user.id);
    } else {
      await supabase.from("attendees").insert({ event_id: circle.eventId, profile_id: session.user.id });
    }
    loadCircles();
  };
  ...
  const createCircle = async (data) => {
    ...
    const { data: circle, error } = await supabase
      ...
      .single();
    if (error) {
      flash("Couldn't post that circle — try again");
      return;
    }
    const { data: event, error: eventError } = await supabase
      .from("events")
      .insert({ circle_id: circle.id, starts_at: data.when })
      .select()
      .single();
    if (!eventError && event) {
      await supabase.from("attendees").insert({ event_id: event.id, profile_id: session.user.id });
    }
    setShowCreate(false);
  ...
  const user = session?.user
    ? { name: profile?.full_name || session.user.email || "You", id: session.user.id }
    : null;

  if (!isSupabaseConfigured) {
    return (
      <div className="wrap" style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ maxWidth: 440, textAlign: "center" }}>
          <h1 style={{ fontSize: 22, marginBottom: 12 }}>Supabase isn't configured yet</h1>
          <p style={{ color: "rgba(242,239,233,0.72)", lineHeight: 1.5 }}>
    ...
    <div className="wrap">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .avatar { width: 30px; height: 30px; border-radius: 50%; background: linear-gradient(135deg,#8B5FBF,#5A8FD6); display:flex; align-items:center; justify-content:center; font-size:12px; font-weight:700; cursor: pointer; border: none; }
        ...
        .modal { background: #241C3D; width: 100%; max-width: 480px; border-radius: 20px 20px 0 0; padding: 24px 20px 28px; max-height: 88vh; overflow-y: auto; border: 1px solid rgba(242,239,233,0.1); position: relative; }
      ...
      <div className="topbar">
        ...
        {loadingUser ? null : user ? (
          <div className="user-chip">
            <button className="avatar" onClick={() => setShowSettings(true)} title="Edit your area">
      ...
      <div className="board">
        ...
        {visible.map((c) => {
          ...
          const joined = user && c.joinedIds.includes(user.id);
          return (
            <div className="card" key={c.id} style={{ "--accent": info.color }}>
              ...
              <div className="card-footer">
                <div>
                  <div className="when-badge">{c.when ? timeUntil(c.when) : "No upcoming date"}</div>
                  <div className="join-count">
                    <Users size={11} style={{ verticalAlign: "-1px", marginRight: 4 }} />
                    {c.joinedNames.length} going · hosted by {c.host}
                  </div>
                </div>
                <button
...
function CreateModal({ onClose, onCreate }) {
  ...
  const canSubmit = title.trim() && description.trim() && location.trim() && when && !submitting;
...
function SettingsModal({ initialArea, onClose, onSave }) {
  const [area, setArea] = useState(initialArea || AREAS[0]);
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <h2>Your area</h2>
        <div className="sub">Used to show you circles nearby. You can change this any time.</div>
        ...
        <div className="modal-actions">
          <button className="btn btn-ghost" onClick={onClose}>Cancel</button>
          <button className="btn btn-primary" onClick={() => onSave(area)}>Save</button>
        </div>
      </div>
    </div>
  );
